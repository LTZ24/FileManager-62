...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    requireRateLimit('forms_add', null, null, BASE_URL . '/pages/forms/index');
    ...
    if (empty($title) || empty($url) || empty($category)) {
        ...
        try {
            if (addFormToSheets($title, $url, $category)) {
                ...
                $categoryKey = 'category_' . $category . '_forms';
                ...
                if ($isModal) {
                    ...
                    header("refresh:2;url=./?success=Form berhasil ditambahkan&category=" . $category);
                }
            } else {
                $sheetName = getFormSheetName($category);
                $detail = $_SESSION['last_sheets_error'] ?? '';
                $error = "Gagal menambahkan form! Pastikan tab '$sheetName' ada di Google Sheets.";
                if (!empty($detail)) {
                    $error .= " Detail: " . $detail;
                }
                if (isAjaxRequest()) {
