...
// Function to get links from category
function getCategoryLinks($sheetId, $category) {
    try {
        $sheetsService = getSheetsService();
        
        $sheetTitle = 'Links-' . ucfirst($category);
...
$links = getCategoryLinks(constant('SHEETS_' . strtoupper($category)), $categoryName);
...
if (!$link) {
    echo '<div style="padding: 2rem; text-align: center; color: #ef4444;">Link tidak ditemukan (ID: ' . htmlspecialchars($id) . ', Category: ' . htmlspecialchars($category) . ')</div>';
    exit;
}
$error = '';
$success = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    requireRateLimit('links_edit_ctr');
    ...
    if (empty($title) || empty($url)) {
        ...
        try {
            // Use ID (0-based index) - updateLinkInSheets will add 2 to get row number
            if (updateLinkInSheets($link['id'], $title, $url, $categoryName)) {
                // Clear cache
                ...
                echo '<script>
                    setTimeout(function() {
                        window.parent.postMessage("link_updated", "*");
                    }, 1500);
                </script>';
            } else {
    ...
    <div class="form-container">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                <label for="title"><i class="fas fa-heading"></i> Judul Link</label>
